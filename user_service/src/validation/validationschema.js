...
const registration_schema=joii.object({//prop must match to reqbody not necesrari to schema as we can change to it in rpocessing
    username : joii.string().min(8).max(20).required(),
    email: joii.string().email().required(),
    password: joii.string().min(8).required(),

})
const login_schema=joii.object({//prop must match to reqbody not necesrari to schema as we can change to it in rpocessing
